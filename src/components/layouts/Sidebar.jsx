...
const linkClass = 'flex items-center gap-2 font-light px-3 py-2 hover:bg-green-800 hover:no-underline hover:text-white focus:bg-gray-800 active:bg-gray-800 rounded-lg';
...
const Sidebar = () => {
    ...
    return (
        <div className='bg-gray-900 flex flex-col min-w-[300px] min-h-screen'>
            <div className='flex items-center gap-2 px-6 py-5 bg-gray-950 flex-shrink-0'>
            ...
            <div className='flex flex-1 flex-col px-5 gap-2 py-2 bg-gray-900'>
                {DASHBOARD_SIDEBAR_LINKS.map((link) => (
                    <SidebarLink key={link.key} link={link} />
                ))}
            </div>
            <div className='flex flex-col px-5 gap-2 py-2 pt-5 border-t border-white bg-gray-900'>
...
const SidebarLink = ({ link }) => {
    ...
    return (
        <Link to={link.path} className={classNames(pathname === link.path ? 'bg-gray-800 text-white' : 'text-neutral-200', linkClass)}>
            <span className=''>
